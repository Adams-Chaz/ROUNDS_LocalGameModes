...
namespace LGM
{
    internal static class PreGamePickBanHandler
    {
        ...
        internal static IEnumerator PreGamePicksStandard(IGameModeHandler gm)
        {
            ...
            if (pregamepickfinished || !LGMMod.PreGamePickMethod || LGMMod.PreGamePickStandard <= 1)
            {
                yield break;
            }
            ...
            for (int _ = 0; _ < LGMMod.PreGamePickStandard - 1; _++)
            {
        ...
        internal static IEnumerator PickFromRarity(Player player, CardInfo.Rarity rarity)
        {
            ...
            switch (rarity)
            {
                case CardInfo.Rarity.Common:
                    cardsToPick = LGMMod.PreGamePickCommon;
                    rarityString = "COMMON";
                    break;
                case CardInfo.Rarity.Uncommon:
                    cardsToPick = LGMMod.PreGamePickUncommon;
                    rarityString = "UNCOMMON";
                    break;
                case CardInfo.Rarity.Rare:
                    cardsToPick = LGMMod.PreGamePickRare;
                    rarityString = "RARE";
                    break;
            }
            if (cardsToPick == 0)
            ...
            // tell LGM to skip the first standard pick phase since the whole-deck pick was used
        ...
        {
            if (pregamepickfinished || LGMMod.PreGamePickMethod || LGMMod.PreGamePickCommon < 1)
            {
                yield break;
            }
        ...
        internal static IEnumerator PreGamePicksUncommon(IGameModeHandler gm)
        {
            ...
            if (pregamepickfinished || LGMMod.PreGamePickMethod || LGMMod.PreGamePickUncommon < 1)
            {
                yield break;
            }
        ...
        internal static IEnumerator PreGamePicksRare(IGameModeHandler gm)
        {
            ...
            if (pregamepickfinished || LGMMod.PreGamePickMethod || LGMMod.PreGamePickRare < 1)
            {
                yield break;
            }
            ...
            for (int p = 0; p < PlayerManager.instance.players.Count; p++)
            {
                ...
                for (var i = 0; i < actions.Length; i++)
                {
                    ...
                    actions[i] = () =>
                    {
                        ...
                        {
                            if (currentBans < LGMMod.PreGameBan)
                            {
                ...
                while (picking)
                {
                    if (PlayerManager.instance.players[p].data.view.ControllerActorNr == PhotonNetwork.LocalPlayer.ActorNumber)
                    {
                        // make sure the menu stays open
                        ToggleCardsMenuHandler.Open(true, true, actions, disabledNames.ToArray().Concat(locallyBanned.ToArray()).ToArray());
                        text.text = "BAN " + (LGMMod.PreGameBan - currentBans).ToString() + " CARD" + ((LGMMod.PreGameBan - currentBans != 1) ? "S" : "");
                    }
                    yield return null;
                }
                yield return new WaitForSecondsRealtime(0.5f);
