...
namespace LGM
{
    ...
    public class DebugWindow : MonoBehaviour
    {
        ...
        public void OnGUI()
        {
            ...
            this.windowRect = GUI.Window(0, this.windowRect, WindowFunction, "LGM Debug");
        }
        ...
        public void WindowFunction(int windowID)
        {
            ...
            float points = builder.NextElement(150, 10).HorizontalSlider(LGMMod.instance.debugOptions.points, 1f, 5f);
            builder.NextElement(40, 10, 12).Label($"{LGMMod.instance.debugOptions.points}", TextAnchor.MiddleRight);

            builder.NextRow(35, 10, 0);
            builder.NextElement(70, 10, 0).Label("rounds");
            float rounds = builder.NextElement(150, 10).HorizontalSlider(LGMMod.instance.debugOptions.rounds, 1f, 5f);
            builder.NextElement(40, 10, 12).Label($"{LGMMod.instance.debugOptions.rounds}", TextAnchor.MiddleRight);
            ...
            if (testerBtn)
            {
                var tester = LGMMod.instance.gameObject.GetComponent<PhotonLagSimulationGui>();
                tester.enabled = !tester.enabled;
            }

            if (PhotonNetwork.OfflineMode || PhotonNetwork.IsMasterClient || PhotonNetwork.CurrentRoom == null)
            {
                if (Mathf.RoundToInt(points) != LGMMod.instance.debugOptions.points)
                {
                    LGMMod.instance.debugOptions.points = Mathf.RoundToInt(points);
                    LGMMod.instance.SyncDebugOptions();
                }

                if (Mathf.RoundToInt(rounds) != LGMMod.instance.debugOptions.rounds)
                {
                    LGMMod.instance.debugOptions.rounds = Mathf.RoundToInt(rounds);
                    LGMMod.instance.SyncDebugOptions();
                }
            }
